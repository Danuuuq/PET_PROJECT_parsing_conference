...
class ConferenceSpider(scrapy.Spider):
    ...
    # url = f"""http://vcs.gazsvyaz.gazprom.ru/issues?c%5B%5D=due_date&c%5B%5D=cf_6&c%5B%5D=cf_7&c%5B%5D=subject&c%5B%5D=cf_5&c%5B%5D=description&f%5B%5D=status_id&f%5B%5D=due_date&f%5B%5D=&group_by=&op%5Bdue_date%5D=%3D&op%5Bstatus_id%5D=o&per_page=50&set_filter=1&utf8=%E2%9C%93&v%5Bdue_date%5D%5B%5D={current_time}"""
    ...
    def parse(self, response):
        ...
        for row in all_conf:
            if row.css('.issue::attr(id)').get():
                ...
                after_call = row.css('em::text').re(r'^.*?\s[Ии]нвест»\s*\(к.*')
                if len(room) == 0:
                    conf = {}
                else:
                    # breakpoint()
